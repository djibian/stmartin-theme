...
/*
 * Ajouter la case à cocher pour signaler que le produit est disponible uniquement sur commande
 */
add_action('woocommerce_product_options_sku', 'add_oncommand_checkbox');
function add_oncommand_checkbox(){
	global $post;

	$value=get_post_meta($post->ID,'_on_command_only', true);
	woocommerce_wp_checkbox(
		array(
			'id'            => '_on_command_only',
			'value'         => $value ? 'yes' : 'no',
			'label'         => __( 'On command only?', 'woocommerce' ),
			'description'   => __( 'Display "on command only" on product photo', 'woocommerce' ),
		)
	);
}

/* Sauvegarde de l'état de la case à cochée */
add_action( 'woocommerce_process_product_meta', 'add_oncommand_checkbox_save' );
function add_oncommand_checkbox_save( $post_id ){
	$onComandCheckbox = isset( $_POST['_on_command_only'] ) ? 'yes' : 'no';
	update_post_meta( $post_id, '_on_command_only', $onComandCheckbox );
}


/*
 * Ajouter l'indication "sur commande" au niveau de l'image du produit
 */
add_action( 'woocommerce_before_shop_loop_item_title', 'display_oncommand_headband', 10 );
add_action( 'woocommerce_before_single_product_summary', 'display_oncommand_headband', 10);
function display_oncommand_headband () {
	global $product;

	$oncommand = get_post_meta($product->get_id(),'_on_command_only', true);
	if ($oncommand) {
		echo '<span class="oncommandonly">' . esc_html__('On command only', 'bocalenbalade') . '</span>';
	}
}